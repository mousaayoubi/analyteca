...
function clampRange({ from, to, maxDays = 90 }) {
	...
	if (diffDays > maxDays) {
		...
		return { fromIso: isoDate(s2), toIso: isoDate(end), clamped: true};
	} 
...
export async function runSync({ from, to, triggeredBy }) {
	...
	const run = await query(
		...
		VALUES ('running', $1)
		RETURNING id`,
		[`Triggered by ${triggeredBy}. Range ${fromIso}..${toIso}${clamped ? " (clamped)" : ""}`]
	);
...
`UPDATE sync_runs SET status='success', finished_at=now() WHERE id=$1`,
	[runId]
		...
		return { ok: true, runId, range: { from: fromIso, to: toIso } };} catch (err) {
			await query(
